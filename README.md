# ExtrarrFin 🎬

> **⚠️ AI-Generated Project Warning**  
> This project was generated by AI in the spirit of "vibe coding" - an experimental approach to rapid development. While functional, the codebase may contain unconventional patterns, incomplete error handling, or areas that could benefit from refactoring.

**ExtrarrFin** is a Python tool that automates the download of special episodes (Season 0), behind-the-scenes extras, and musical themes for your Sonarr series and Radarr movies, using yt-dlp to search and download content from YouTube.

---

## ✨ Features

- 🔍 **Automatic detection** — retrieves all monitored series with monitored Season 0
- 📺 **YouTube download** — uses yt-dlp to download special episodes
- 🎯 **Smart video matching** — intelligent scoring system ([see SCORING.md](SCORING.md))
- 🏃 **Dry-run mode** — simulate without downloading
- ♻️ **Duplicate detection** — skips already-downloaded files (`--force` to override)
- 🔄 **Sonarr/Radarr integration** — triggers a scan after download
- 🎬 **Radarr support** — download extras for movies (tag mode)
- 🏷️ **Tag mode** — download behind-the-scenes via `want-extras` tag ([see TAG_MODE.md](TAG_MODE.md))
- 🎵 **Theme music** — downloads `theme.mp3` for every series and movie
- 📝 **Subtitles** — automatic download and SRT conversion
- 📺 **STRM mode** — streaming files instead of downloads (saves disk space)
- ⏰ **Schedule mode** — periodic automatic downloads
- 🐳 **Docker support** — Debian-based container image
- 📂 **Directory mapping** — remote execution with path remapping
- 🧹 **Auto cleanup** — removes incomplete `.part` files

---

## 🚀 Quick start

```bash
# 1. Install
git clone https://github.com/yourusername/extrarrfin.git && cd extrarrfin
pip install -r requirements.txt

# 2. Configure
cp config.example.yaml config.yaml && nano config.yaml

# 3. Test
python extrarrfin.py test

# 4. Dry-run
python extrarrfin.py download --dry-run

# 5. Download
python extrarrfin.py download
```

---

## 📚 Documentation

| Page | Description |
|------|-------------|
| [docs/installation.md](docs/installation.md) | Prerequisites, installation (standard & Docker) |
| [docs/configuration.md](docs/configuration.md) | All config options (YAML, env vars, CLI) |
| [docs/usage.md](docs/usage.md) | All CLI commands with examples and common use cases |
| [docs/season0.md](docs/season0.md) | Season 0 mode — what gets downloaded and how |
| [TAG_MODE.md](TAG_MODE.md) | Tag mode — behind-the-scenes extras |
| [docs/theme-mode.md](docs/theme-mode.md) | Theme music mode — `theme.mp3` download |
| [docs/subtitles.md](docs/subtitles.md) | Subtitle management |
| [docs/strm-mode.md](docs/strm-mode.md) | STRM file mode (streaming, no disk usage) |
| [docs/jellyfin.md](docs/jellyfin.md) | Jellyfin integration |
| [docs/docker.md](docs/docker.md) | Docker & Docker Compose |
| [docs/advanced.md](docs/advanced.md) | Directory mapping, systemd, cron, troubleshooting |
| [SCORING.md](SCORING.md) | Video scoring system — configuration and tuning |

---

## 📋 Prerequisites

- Python 3.8+
- Sonarr instance with API key (required)
- Radarr instance with API key (optional, for movies)
- FFmpeg

→ [Full installation guide](docs/installation.md)

---

## ⚠️ Disclaimer

Use responsibly and in accordance with YouTube's Terms of Service and the copyright laws of your country.

---

## 🙏 Acknowledgments

[Sonarr](https://sonarr.tv/) · [Radarr](https://radarr.video/) · [yt-dlp](https://github.com/yt-dlp/yt-dlp) · [Click](https://click.palletsprojects.com/) · [Rich](https://github.com/Textualize/rich)

---

**Made with ❤️ for the *arr community**
